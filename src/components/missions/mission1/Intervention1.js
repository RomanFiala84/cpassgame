...
const tasks = [
  { id: 't1', title: 'Kritické myslenie', question: 'Aké otázky by ste si položili pri hodnotení informácií?' },
  { id: 't2', title: 'Zaujatosti', question: 'Ako identifikujete zaujatý text alebo manipulatívny obsah?' }
];

export default function Intervention1() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const saved = progress?.intervention1_data || { taskAnswers: {}, completedTasks: {} };
      setAnswers(saved.taskAnswers);
      setCompleted(saved.completedTasks);
    })();
    return () => {
      const duration = Date.now() - startTime;
      dataManager.saveComponentData(userId, 'intervention1', { duration });
    };
  ...
  const handleComplete = async (id, checked) => {
    ...
    const progress = await dataManager.loadUserProgress(userId);
    const cur = progress.intervention1_data || { taskAnswers: {}, completedTasks: {} };
    cur.taskAnswers = { ...cur.taskAnswers, [id]: answers[id] };
    cur.completedTasks = { ...cur.completedTasks, [id]: checked };
    progress.intervention1_data = cur;
  ...
  return (
    <Layout>
      <Container>
        <Card>
          <Title>Intervenčný tréning</Title>
          <Description>
