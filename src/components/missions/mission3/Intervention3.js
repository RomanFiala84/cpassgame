...
const tasks = [
  { id: 't1', title: 'Fact-checking', question: 'Ako overíte pravdivosť tvrdenia?' },
  { id: 't2', title: 'Logické úsudky', question: 'Ako rozpoznáte logické chyby v argumentácii?' }
];
...
const Intervention3 = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const saved = (progress && progress['intervention3_data']) || { taskAnswers: {}, completedTasks: {} };
  ...
  const handleComplete = (id, checked) => {
    ...
    (async () => {
      const progress = await dataManager.loadUserProgress(userId);
      const cur = progress['intervention3_data'] || { taskAnswers: {}, completedTasks: {} };
      ...
      progress['intervention3_data'] = cur;
  ...
  return (
    <Layout>
      <Container>
        <Card>
          <Title>Intervenčný tréning - Misia 3</Title>
          <Description>
