...
const Questionnaire2B = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      // OPRAVA: Špecifický kľúč pre 2B
      const saved = (progress && progress['questionnaire2b_data']) || {};
  ...
  const handleContinue = async () => {
    ...
    const progress = await dataManager.loadUserProgress(userId);
    progress['questionnaire2b_data'] = { answers, timestamp: new Date().toISOString() };
  ...
  return (
    <Layout>
      <Container>
        <Card>
          <Title>Dotazník 2B – Finálna dôvera</Title>
