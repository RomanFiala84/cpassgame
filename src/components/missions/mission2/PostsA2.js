...
const mockPostsA2 = [
  { id: 1, username: 'user_m2_a1', content: 'Mission 2 Post A2-1.', image: null },
  { id: 2, username: 'user_m2_a2', content: 'Mission 2 Post A2-2.', image: '/img/a2-2.jpg' },
  { id: 3, username: 'user_m2_a3', content: 'Mission 2 Post A2-3.', image: '/img/a2-3.jpg' }
];
...
const PostsA2 = () => {
  ...
  useEffect(() => {
    (async () => {
      if (userId) {
        ...
        const saved = (progress && progress['postsA2_data']) || {};
        setRatings(saved);
      }
    })();
  }, [userId, dataManager]);
  ...
  const handleRating = (id, value) => {
    ...
    (async () => {
      const progress = await dataManager.loadUserProgress(userId);
      const cur = (progress && progress['postsA2_data']) || {};
      cur[id] = value;
      cur.timestamp = new Date().toISOString();
      progress['postsA2_data'] = cur;
      await dataManager.saveProgress(userId, progress);
    })();
  };

  const handleContinue = async () => {
    const missing = mockPostsA2.map(p => p.id).filter(id => !ratings[id]);
    if (missing.length) {
    ...
    if (group === '1') navigate('/mission2/intervention');
    else navigate('/mission2/postsb');
  };

  return (
    <Layout>
      <Container>
        <Title>Hodnotenie príspevkov A - Misia 2</Title>
        <PostsGrid>
          {mockPostsA2.map(post => (
            <PostCard 
              key={post.id} 
